...
exports.passwordMail = (email, password) =>
  sendgrid
    ...
    .then(() => console.log(`New password sent to ${email}`))
    .catch((err) => {
      console.log('[sendgrid', err);
      throwError('Došlo je do greške prilikom slanja nove šifre.', 500);
    });
...
exports.sendVerificationMail = (token, email) =>
  sendgrid
    ...
    .catch((err) => {
      ...
      console.log('[sendgrid', err);
      throwError(
        'Došlo je do greške prilikom slanja verifikacionog mejla.',
        500
      );
    });
